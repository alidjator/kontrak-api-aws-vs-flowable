...
| 2 | Channel PUSH pelengkap, bukan satu-satunya sumber | Sama seperti Endpoint A, callback ini adalah channel **PUSH pelengkap** — progres approval bisa dipantau lewat PULL kapan pun SEBELUM semua approver selesai, tapi lewat variabel per-approver `hasilKeputusan_<grup>` (satu variabel per approver yang SUDAH selesai — lihat [KONTRAK-API-FLOWABLE.md §3.2 poin 5](./KONTRAK-API-FLOWABLE.md#32-variabel-proses-tambahan-includeprocessvariables)/[§4.1](./KONTRAK-API-FLOWABLE.md#41-variabel-yang-wajib-dikirim-saat-complete)/[§5.4](./KONTRAK-API-FLOWABLE.md#54-ambil-variabel-proses-langsung)), **BUKAN** array gabungan `hasilApprovalBod` — array itu cuma dibangun SEKALI, tepat sebelum callback ini sendiri dikirim, dan TIDAK PERNAH tersimpan sebagai variabel proses yang bisa dibaca lebih awal (beda dari desain awal proses ini yang sempat begitu, tapi terbukti menimbulkan race condition di produksi — lihat RINGKASAN-PEMBAHASAN.md Update 66) |
